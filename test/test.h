...
#define ASSERT_TRUE(cond)					\
	if (!(cond)) {						\
		warning_bs("selftest: ASSERT_TRUE: %s:%u:\n",	\
			__FILE__, __LINE__);			\
		err = EINVAL;					\
		goto out;					\
	}
...
#define ASSERT_EQ(expected, actual)				\
	if ((expected) != (actual)) {				\
		warning_bs("selftest: ASSERT_EQ: %s:%u: %s():"	\
...
#define ASSERT_DOUBLE_EQ(expected, actual, prec)			\
	if (!test_cmp_double((expected), (actual), (prec))) {		\
		warning_bs("selftest: ASSERT_DOUBLE_EQ: %s:%u:"		\
			" expected=%f, actual=%f\n",			\
			__FILE__, __LINE__,				\
...
#define ASSERT_STREQ(expected, actual)					\
	if (0 != str_cmp((expected), (actual))) {			\
		warning_bs("selftest: ASSERT_STREQ: %s:%u:"		\
			" expected = '%s', actual = '%s'\n",		\
			__FILE__, __LINE__,				\
			(expected), (actual));				\
		err = EBADMSG;						\
...
#define ASSERT_PLEQ(expected, actual)					\
	if (0 != pl_cmp((expected), (actual))) {			\
		warning_bs("selftest: ASSERT_PLEQ: %s:%u:"			\
			" expected = '%r', actual = '%r'\n",		\
			__FILE__, __LINE__,				\
			(expected), (actual));				\
		err = EBADMSG;						\
