...
static bool handle_register(struct sip_server *srv, const struct sip_msg *msg)
{
	...
	if (err) {
		if (err == ENOENT) {
			warning_bs("domain not found\n");
			return false;
		}

		sip_reply(sip, msg, 500, strerror(err));
		warning_bs("domain find error: %s\n", strerror(err));
		return true;
	}
	...
	for (le=list_head(lst); le; le=le->next) {
		...
		if (err) {
			sip_reply(sip, msg, 500, strerror(err));
			if (err != EPROTO)
				warning_bs("location update error: %s\n",
				      strerror(err));
			goto fail;
		}
	}
